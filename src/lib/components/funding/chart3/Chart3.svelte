<script>
	...
	import employerContPct from '$lib/data/Funding/employer_cont_pct.js';
	...
	let points1 = employerContPct[selectedKey1];
	let points2 = employerContPct[selectedKey2];
	let points3 = employerContPct[selectedKey3];
	let points4 = employerContPct[selectedKey4];
	...
	const xExtent = extent(
		[
			...employerContPct[selectedKey1],
			...employerContPct[selectedKey2],
			...employerContPct[selectedKey3],
			...employerContPct[selectedKey4]
		],
		(d) => new Date(d.x)
	);
	...
	function setTween1(key) {
		tweenedPoints1.set(employerContPct[key]);
	}

	function setTween2(key) {
		tweenedPoints2.set(employerContPct[key]);
	}

	function setTween3(key) {
		tweenedPoints3.set(employerContPct[key]);
	}

	function setTween4(key) {
		tweenedPoints4.set(employerContPct[key]);
	}
	...
	const getYValue1 = (date) =>
		employerContPct[selectedKey1].filter((d) => new Date(d.x) >= date)[0]?.y;
	const getYValue2 = (date) =>
		employerContPct[selectedKey2].filter((d) => new Date(d.x) >= date)[0]?.y;
	const getYValue3 = (date) =>
		employerContPct[selectedKey3].filter((d) => new Date(d.x) >= date)[0]?.y;
	const getYValue4 = (date) =>
		employerContPct[selectedKey4].filter((d) => new Date(d.x) >= date)[0]?.y;
		...
		<svg {width} {height}>
			<g transform="translate({margin.left} {margin.top})">
				...
				<HoverEvents
					...
					{margin}
					{maxDate}
					data1={employerContPct}
					data2={employerContPct}
					data3={employerContPct}
					data4={employerContPct}
					{color1}
					{color2}
					{color3}
					{color4}
