<script>
	...
	import ualData from '$lib/data/Funding/ual.js';
	import HoverEvents from '$lib/components/funding/chart2/HoverEvents.svelte';
	...
	let points1 = ualData[selectedKey1];
	let points2 = ualData[selectedKey2];
	let points3 = ualData[selectedKey3];
	let points4 = ualData[selectedKey4];
	...
	const xExtent = extent(
		[
			...ualData[selectedKey1],
			...ualData[selectedKey2],
			...ualData[selectedKey3],
			...ualData[selectedKey4]
		],
		(d) => new Date(d.x)
	);
	...
	function setTween2(key) {
		tweenedPoints2.set(ualData[key]);
	}

	function setTween3(key) {
		tweenedPoints3.set(ualData[key]);
	}

	function setTween4(key) {
		tweenedPoints4.set(ualData[key]);
	}
	...
	const getYValue1 = (date) => ualData[selectedKey1].filter((d) => new Date(d.x) >= date)[0]?.y;
	const getYValue2 = (date) => ualData[selectedKey2].filter((d) => new Date(d.x) >= date)[0]?.y;
	const getYValue3 = (date) => ualData[selectedKey3].filter((d) => new Date(d.x) >= date)[0]?.y;
	const getYValue4 = (date) => ualData[selectedKey4].filter((d) => new Date(d.x) >= date)[0]?.y;
