<script>
	...
	import benefitsData from '$lib/data/Benefit/benefits_indexed.js';
	...
	{#if tooltipData}
		...
		>
			...
			{#if checkboxOne}
				...
				/>
				Line 1: {format('$,.0f')(getYValue1(hoveredDate))}<br />
			{/if}
			{#if checkboxTwo}
				<span
					style="width: 9px;
				...
				/>
				Line 2: {format('$,.0f')(getYValue2(hoveredDate))}<br />
			{/if}
			{#if checkboxThree}
				<span
					style="width: 9px;
				...
				/>
				Line 3: {format('$,.0f')(getYValue3(hoveredDate))}<br />
			{/if}
			{#if checkboxFour}
				<span
					style="width: 9px;
			...
			border-radius: 50%;background: {color4};"
				/>
				Line 4: {format('$,.0f')(getYValue4(hoveredDate))}
			{/if}
		</div>
	{/if}
